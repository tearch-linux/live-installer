...
class InstallerEngine:
    ...
    def start_installation(self):
        ...
        self.do_run_in_chroot(
            'useradd -m -g users -G wheel -s /bin/bash {username}'.format(username=self.setup.username))
        self.do_run_in_chroot(
            ...
            if(partition.mount_as is not None and partition.mount_as != ""):
                ...
                if partition.mount_as == "/@":
                    if partition.type != "btrfs":
                        self.error_message(
                            message=_("ERROR: the use of @subvolumes is limited to btrfs"))
                        return
    ...
    def finish_installation(self):
        ...
        kernelversion = subprocess.getoutput("uname -r")
        self.do_run_in_chroot(
            "/usr/sbin/mkinitcpio -g /boot/initramfs-"+kernelversion+".img")
        self.do_run_in_chroot(
            "/usr/sbin/mkinitcpio -g /boot/initramfs-"+kernelversion+"-fallback.img")
